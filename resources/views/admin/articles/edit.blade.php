...
<div class="max-w-3xl rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
    @if ($article->thumbnail)
        <img src="{{ asset('storage/' . $article->thumbnail) }}" class="mb-4 h-48 w-full rounded-xl object-cover" alt="">
    @endif
    <form method="POST" action="{{ route('admin.articles.update', $article) }}" enctype="multipart/form-data" class="grid gap-4 sm:grid-cols-2">
        ...
        <div><label class="mb-1 block font-mono text-[11px] uppercase tracking-[0.09em] text-gray-500">Kategori *</label><input name="category" value="{{ old('category', $article->category) }}" required class="w-full rounded-md border-gray-300 text-sm text-gray-900 placeholder-gray-400 focus:border-gray-500 focus:ring-0"></div>
